...
debug.enable('mongodb-prebuilt');
debug = debug('mongodb-prebuilt');

var mongodb_logs =require('debug');
mongodb_logs.enable('mongodb');
mongodb_logs = mongodb_logs('mongodb');
...
function start_server(opts, callback) {
	...
	var spawn = (opts.args.fork) ? spawnSync : child_process.spawn;
        var child = spawn(bpath + "mongod", args);//spawnSync(bpath + "mongod", args);
        child.stdout.on('data', function(data) {
	    mongodb_logs(data);
	    debug('mongodb stdout: '+data);
	});
        child.stderr.on('data', function(data) {
            mongodb_logs(data);
	    debug('mongodb stderr: '+ data);
	});

        // error
        child.on('error', function(err) {
	    mongodb_logs('Failed to start child process.');
	    debug('Failed to start child mongodb process.');
            if (opts.exit_callback) {
                opts.exit_callback(child.pid);
            }
	...
	if(child.pid) { //mongod started
	  //need to catch child pid
	  var child_pid_match = child.pid;//child.stdout.toString().match(/forked process:\s+(\d+)/i);
	  //child_pid = child_pid_match[1];
